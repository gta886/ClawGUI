...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        #     pprint.pprint(batch, depth=3, width=120)
        #     print("=" * 50)
        #     break
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Periodic container restart for MobileWorld environments
                env_restart_cfg = self.config.env.get('restart', {})
                env_restart_enable = env_restart_cfg.get('enable', False) if env_restart_cfg else False
                if env_restart_enable and self.envs is not None and hasattr(self.envs, 'restart_all_containers'):
                    restart_every_n_steps = env_restart_cfg.get('every_n_steps', 10)
                    if self.global_steps % restart_every_n_steps == 0:
                        wait_time = env_restart_cfg.get('wait_after_run', 360)
                        print(f"\n{'='*60}")
                        print(f"[Trainer] Periodic container restart triggered at step {self.global_steps} "
                              f"(every {restart_every_n_steps} steps)")
                        print(f"{'='*60}\n")
                        restart_ok = self.envs.restart_all_containers(
                            wait_after_run=wait_time,
                        )
                        if restart_ok:
                            print(f"[Trainer] Container restart completed successfully")
                        else:
                            print(f"[Trainer] WARNING: Container restart had issues, some servers may be unhealthy")
